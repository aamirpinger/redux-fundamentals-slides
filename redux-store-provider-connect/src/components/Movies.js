...
// this is the example of mergeProps property of connect function
// mergeProps is where you can filter the props being passed to the component
// here in below function for explaination pupose I have added a single record 
// of movie name 'Mission impossible if the state from the store is empty 
const mergeProps = (propsFromState, propsFromDispatch, ownProps) => {
    return {
         movies: (propsFromState.movies.length !== 0) 
                 ? propsFromState.movies 
                 : [{id:0, name: "Mission impossible", done: true}],
         toggle_movie:  propsFromDispatch.toggle_movie,
         addMovie: propsFromDispatch.addMovie,
         deleteMovie: propsFromDispatch.deleteMovie
    }
}
...
export default connect((state) => ({
    movies: state.movies
}), mapDispatchToProps,mergeProps)(Movies)
